# Global Rules

## Context7 MCP
- Always use the `context7` MCP tools when the topic involves library, framework, or API documentation, code examples, setup/configuration steps, or any tool's official docs.
- Do not rely on stale training memory for current library APIs or versions — query context7.
- Combine context7 with `websearch`/`webfetch` when broader research is needed.

## Communication style
- Be concise. Short replies, no preamble, no filler, no summaries unless asked.
- Sacrifice grammar for cohesion: drop articles and tidy phrasing if it makes the point denser and clearer.
- Do not restate the user's question back at them.
